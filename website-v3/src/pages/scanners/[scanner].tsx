...
const ScannerPageInner: NextPage<InferGetStaticPropsType<typeof getStaticProps>> = (props) => {
  ...
  if (selectedClass === undefined) {
    return (
      <>
        <Navbar></Navbar>
        Choose a class before continuing: {classes.isLoading && "Loading..."}
        {classes.isError && (
          <button
            onClick={() => {
    ...
    <div>
        ...
        (classes.data.length === 0 ? (
          ...
          <Select
            ...
            defaultValue={{
              ...
              value: classes.data.find((a) => a.id === selectedClass)!.id,
              // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
              label: classes.data.find((a) => a.id === selectedClass)!.name,
            }}
          />
        ))}
      <br />
      ...
      <Select
        ...
        defaultValue={{
          ...
          value: "realtime" as "realtime" | "date-to-realtime" | "date-to-date",
        }}
      />
      {(mode === "date-to-realtime" || mode === "date-to-date") && (
        <div>
          Choose your start time: <DatePicker time={startDate} setTime={setStartDate} />
        </div>
      )}
      {mode === "date-to-date" && (
        <div>
          Choose your end time: <DatePicker time={endDate} setTime={setEndDate} />
        </div>
      )}
      {(mode !== "realtime" ? signedInD.isLoading : false) || people.isLoading ? (
        <p>Loading...</p>
      ) : (
        people.isSuccess && (
          <div className='flex flex-row space-between'>
            ...
            {/* <br /> */}
            <Table
              isLoading={(mode !== "realtime" ? signedInD.isLoading : false) || people.isLoading}
              data={signedIn
                .filter((value) => {
                ...
                .map((value) => {
                  ...
                  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                  return { ...people.data.get(value.studentId)!, timestamp: value.timestamp };
                })}
            />
